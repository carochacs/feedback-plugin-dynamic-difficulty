...
- **`feedback-plugin-splitscreen`** (`window.feedBackSplitscreen` / `window.slopsmithSplitscreen`) — verified present as of splitscreen **v1.14.5**. Globals are used to detect and gate whether splitscreen is active before registering per-panel highways; difficulty-ladder maintains the per-panel score state itself, keyed by each highway.
